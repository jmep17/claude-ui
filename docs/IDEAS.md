...
| Idea | Notes |
| :--- | :--- |
| **#6** Tool and MCP usage analytics, with an off switch | `_scan_transcript()` now counts every `tool_use` name (kind `tool`, `CACHE_V` 8), so `mcp__server__tool` pairs split into (server, tool) for free. `tooluse.py` joins that histogram with `permissions.deny` and the MCP inventory; the Insight tab lists every built-in tool and MCP server with measured uses and a **Turn off** that writes a bare tool name into `permissions.deny` — Claude Code's own switch, the settings equivalent of `--disallowedTools`, removing the tool (schema included) from new sessions. Argument-filter rules like `Bash(git:*)` are never touched, and turning back on removes exactly the bare entry. MCP rows reuse `mcp_set_enabled()`, the bigger lever. `toolinfo.py` is the one catalog behind both surfaces: the official tools reference's full list (44, `Agent` through `Write`, checked 2026-08-21) plus the tools current sessions expose before the docs list them, badged *unverified*. It also feeds the `permissions.allow`/`ask`/`deny` pickers, so the rule editors autofill every bare tool name plus live `mcp__<server>` rules. The two documented edge cases are honored: `EndConversation` (a bare-name deny can't remove it) gets no switch and no suggestion, and a legacy name observed in transcripts (`Task`, `BashOutput`, `SlashCommand`…) is labeled rather than offered. The old `mcp__github__*` allow example is gone — `mcp__github` covers the server, allow-rule names take no wildcards (deny/ask do accept glob names like `mcp__*`). |
