...
from .core import atomic_write, config_dir, set_frontmatter_key, tilde
...
def _render(text):
    ...
    return (text
            ...
            .replace("__PROJECTS_TILDE__", tilde(config_dir() / "projects")))
...
def _install_skill(name, payload):
    ...
    if live.is_file():
        ...
        if text != payload and \
                set_frontmatter_key(text, PRESET_KEY, PRESET_REF) == payload:
            atomic_write(live, payload)  # same skill, minus only our stamp
        return                           # already ours, or the user's edit
    parked = resolve_item("skills", name, False, None)
    if parked.exists() or parked.is_symlink():
...
def handoff_state():
    ...
    return {
        ...
        "desc": ("Install the handoff system, vendored in this repo: /handoff "
                 ...
                 "resumes one in place. Apply wires five hook blocks and one "
                 f"permissions entry for {tilde(storep)} so a project-scoped "
                 "session can read the store. Remove unhooks; parked briefs "
                 "are never touched"),
        "installed": installed,
        "detail": detail,
        "target": tilde(hookp),
        ...
        "notes": [
            f"{tilde(hookp)} — the hook and its --list/--take CLI — {hook_note}",
            f"{tilde(_skill_dir('handoff'))}/ — /handoff, writes a brief — "
            f"{status['handoff'][1]}",
            f"{tilde(_skill_dir('handoffs'))}/ — /handoffs, lists and resumes — "
            f"{status['handoffs'][1]}",
            f"{tilde(storep)}/ — the store; created on Apply, never deleted",
